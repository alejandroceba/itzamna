...
# ---------- USER CONFIG ----------
SERIAL_PORT = "/dev/ttyACM0"  # Set your desired port here
BAUD = 460_800
MAX_POINTS = 120
...
def open_serial_with_fallback(port_name: str, baud: int) -> serial.Serial:
    ...
    try:
        # Increased timeout for more robust image transfer
        return serial.Serial(port_name, baud, timeout=2.0)
    ...
    for c in list_candidate_ports():
        ...
        try:
            ...
            return serial.Serial(c.device, baud, timeout=2.0)
        except Exception:
            continue
...
def handle_img_begin_bin(image_id: int, width: int, height: int, data_len: int) -> None:
    ...
    # Flush serial buffers before starting image transfer
    if ser:

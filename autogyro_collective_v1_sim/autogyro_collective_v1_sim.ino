// Synthetic test sketch based on autogyro_collective_v1.ino.
// Keeps the flight-controller PWM inputs and servo outputs unchanged.
// Replaces the BME280 altitude and rotor RPM sensors with generated profiles
// so the control logic can be tested on the bench.
#include <Arduino.h>
...
float computeAdaptiveAlphaDeg(float heightAboveGroundM, float descentRateMps, float rpmNow, float alphaBaseCurveDeg) {
  ...
  // Apply adjustments directly modifying the baseline target geometric sweep profile
...
void updateSyntheticProfile(unsigned long nowMs) {
  ...
  if (nowMs < climbEndMs) {
    ...
    // PHYSICS FIX: Authentic aerodynamic coupling profile. 
    // Minimum pitch (negative alpha) creates less vertical drag, resulting in a higher descent velocity.
    // Positive pitch (flare) generates lift, braking descent rate safely.
    float baseDescentRate = 10.0f; 
    if (!isnan(alpha)) {
      baseDescentRate -= alpha * 0.45f; 
    }
    // Add micro-vibrations to trigger closed-loop controller corrections dynamically
  ...
  for (;;) {
    ...
    if (postDeployMinPitchPending) {
      ...
      if (controlWindowReady) {
        ...
        // CORRECTION: Base adaptive corrections off the flight profile curve instead of getting frozen
        ...
        // CORRECTION: Couple swashplate microseconds directly to closed-loop alpha corrections
    ...
    // CORRECTION: Structured with dedicated final else branch to run mixing algorithm continuously
    if (forceDeployMaxPitch) {
      collectiveBiasUs = (long)pwmMaxUs - (long)collectiveCenterUs;
      servo1AdjustedPwmUs = pwmMinUs;
      servo2AdjustedPwmUs = pwmMaxUs;
      servo3AdjustedPwmUs = pwmMinUs;
    } else if (forcePostDeployMinPitch) {
      // CORRECTION: Clamp exactly to -12 degrees instead of full structural travel limit 
      ...
      // CORRECTION: Actively mixes tracking targets continuously over runtime
      ...
      servo3AdjustedPwmUs = clampPwmUs((long)width3Us - collectiveBiasUs);
    }
